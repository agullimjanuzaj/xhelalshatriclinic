...
export function ManagerPatientsView() {
  const { data: session } = useSession();
  const router = useRouter();
  const queryClient = useQueryClient();
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const [activeInClinic, setActiveInClinic] = useState('');
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['patients', page, search, branchId, activeInClinic],
    queryFn: () => patientsApi.getAll({ page, limit: 24, search, branchId, activeInClinic: activeInClinic || undefined }),
    enabled: !!branchId,
  ...
  const columns = [
    ...
    {
      header: 'Seancat',
      accessor: (row: any) => <span className="text-sm font-medium">{row._count?.sessions ?? 0}</span>,
    },
    {
      header: 'Plane',
      accessor: (row: any) => <span className="text-sm">{row._count?.treatmentPlans ?? 0}</span>,
    },
    {
      header: 'Statusi',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        <SearchInput placeholder="Kërko pacientë..." onChange={(v) => { setSearch(v); setPage(1); }} className="max-w-xs" />
      ...
      <DataTable
        ...
        onRowClick={(row: any) => router.push(getPatientDetailPath('MANAGER', row.id))}
      />
      ...
      <ConfirmDialog
        ...
        description={`Jeni të sigurt që doni të fshini ${deletePatient?.firstName} ${deletePatient?.lastName}? Historiku financiar (seancat, pagesat) nuk humbet.`}
